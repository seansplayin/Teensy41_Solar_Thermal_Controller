...
void serveStaticAssets(AsyncWebServer& server) {
  server.on("/static/*", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (FlashFS.exists(path.c_str())) {
        String ct = getContentType(path);
        File file = FlashFS.open(path.c_str(), FILE_READ);
        if (file) {
...
void serveFavicon(AsyncWebServer& server) {
   ...
   server.on("/favicon.png", HTTP_GET, [](AsyncWebServerRequest *request){
    File f = FlashFS.open("/favicon.png", FILE_READ);
    if (f) {
...
unsigned long calculateTotalLogRuntime(const String& logFilename) {
  ...
  if (!FlashFS.exists(logFilename.c_str())) {
    giveFsMutex();
    return 0;
  }

  File logFile = FlashFS.open(logFilename.c_str(), FILE_READ);
    if (!logFile) {
...
unsigned long aggregatePreviousDailyLogsReport(int pumpIndex, DateTime currentTime) {
  ...
  if (!FlashFS.exists(dailyLogFilename.c_str())) {
    giveFsMutex();
    return 0;
  }

  File dailyLogFile = FlashFS.open(dailyLogFilename.c_str(), FILE_READ);
  if (!dailyLogFile) {
...
unsigned long aggregatePreviousMonthlyLogsReport(int pumpIndex, DateTime currentTime) {
  ...
  if (!FlashFS.exists(monthlyLogFilename.c_str())) {
    giveFsMutex();
    return 0;
  }

  File monthlyLogFile = FlashFS.open(monthlyLogFilename.c_str(), FILE_READ);
  if (!monthlyLogFile) {
...
unsigned long aggregatePreviousYearlyLogsReport(int pumpIndex, DateTime currentTime) {
  ...
  if (!FlashFS.exists(yearlyLogFilename.c_str())) {
    giveFsMutex();
    return 0;
  }

  File yearlyLogFile = FlashFS.open(yearlyLogFilename.c_str(), FILE_READ);
  if (!yearlyLogFile) {
...
unsigned long aggregateDecadeLogsReport(int pumpIndex, DateTime currentTime) {
  ...
  File file = FlashFS.open(filename.c_str(), FILE_READ);
  if (!file || file.size() == 0) {
    if (file) file.close();
...
server.on("/download-log", HTTP_GET, [](AsyncWebServerRequest* request) {
    if (request->hasParam("file")) {
        ...
        if (FlashFS.exists(filePath.c_str())) {
            ...
            File file = FlashFS.open(filePath.c_str(), FILE_READ);
            if (file) {
                request->send(file, ct, file.size());
            } else {
                request->send(404, "text/plain", "Not found");
...
void WebServerManager_begin() {
   // setupNetwork();              // starts Ethernet + registers ALL routes
    //server.begin();
    Serial.println("[Web] WebServerManager_begin() called - no action taken");
}