...
static bool deletePathRecursiveUnlocked(const String& path) {
  ...
  File check = FlashFS.open(p, FILE_READ);
  ...
  bool ok = true;
  if (isDir) {
    File dir = FlashFS.open(p, FILE_READ);
    if (dir) {
      File entry = dir.openNextFile();
      while (entry) {
        String child = path + "/" + String(entry.name());
        ok = deletePathRecursiveUnlocked(child) && ok;
        entry.close();
        entry = dir.openNextFile();
      }
      dir.close();
    }
    ok = FlashFS.rmdir(p) && ok;
  } else {
    ok = FlashFS.remove(p) && ok;
  }
  return ok;
}
...
void setupThirdPageRoutes() {
  ...
  server.on("/temperature-logs/graph", HTTP_GET, [](AsyncWebServerRequest *req) {
    ...
    File f = FlashFS.open(filePath.c_str(), FILE_READ);
    if (!f) {
      xSemaphoreGive(fileSystemMutex);
  ...
  server.on("/fs/list", HTTP_GET, [](AsyncWebServerRequest *req) {
    ...
    File root = FlashFS.open(dir.c_str(), FILE_READ);
  ...
  server.on("/fs/view", HTTP_GET, [](AsyncWebServerRequest *req) {
    ...
    s->f = FlashFS.open(path.c_str(), FILE_READ);
    if (!s->f) {
      delete s;
      xSemaphoreGive(fileSystemMutex);
 ...
 // TarGZ::registerRoutes(server);
  ...
  server.on("/fs/mkdir", HTTP_POST, [](AsyncWebServerRequest *req) {
    ...
    bool created = FlashFS.mkdir(path.c_str());
    xSemaphoreGive(fileSystemMutex);
  ...
  server.on(
    ...
    [](AsyncWebServerRequest *req, String filename, size_t index, uint8_t *data, size_t len, bool final) {
      ...
      if (index == 0) {
        ...
        if (!FlashFS.exists(mk.c_str())) {
          FlashFS.mkdir(mk.c_str());
        }

        uploadFile = FlashFS.open(fullPath.c_str(), FILE_WRITE);
                if (!uploadFile) {
