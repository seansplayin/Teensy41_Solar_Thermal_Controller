...
extern AsyncWebSocket ws;  // Global WebSocket from WebServerManager
...
void sendHeatingCallStatus(bool heatingCall, bool isNormalized) {
  // Simple WS broadcast example
  String msg = "{\"heatingCall\":" + String(heatingCall ? "true" : "false") + 
               ",\"normalized\":" + String(isNormalized ? "true" : "false") + "}";
  
  // Use global WS from WebServerManager
  ws.textAll(msg);
}